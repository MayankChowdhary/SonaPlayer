...
public class Video_View_Adapter_Large extends RecyclerView.Adapter<Video_Holder_Large> implements SectionIndexer {
   ...
   public List<VideoData> list;
    ...
    public Video_View_Adapter_Large(List<VideoData> list, Context context) {
        this.list = list;
    ...
    @Override
    public Video_Holder_Large onCreateViewHolder(ViewGroup parent, int viewType) {
        //Inflate the layout, initialize the View Holder
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.video_listitem_large, parent, false);
        Video_Holder_Large holder = new Video_Holder_Large(v);
        return holder;
    }

    @Override
    public void onBindViewHolder(Video_Holder_Large holder, int position) {
        ...
        holder.songMeta.setText(timeConvert(list.get(position).duration) + " | " + getSongFormat(list.get(position).imageId)+" | "+list.get(position).size);
        ...
        if(checkFlag) {
            holder.mark_box_vl.setVisibility(View.VISIBLE);
            if (!itemStateArray.get(position, false)) {
                holder.mark_box_vl.setChecked(false);}
            else {
                holder.mark_box_vl.setChecked(true);
            }
        }
        else if(holder.mark_box_vl.getVisibility()==View.VISIBLE)
            holder.mark_box_vl.setVisibility(View.GONE);
